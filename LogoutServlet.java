...
public class Listing {
 ...
 // ==========================================
    // START: CUSTOM LISTING CODE LOOK ENHANCEMENT
    // ==========================================
    /**
     * Dynamically generates a display code like DM-00103 based on existing location variables.
     * If names are not present or too short, it handles them gracefully with fallbacks.
     */
    public String getCustomListingCode() {
        // Fallback characters if city or area fields are empty
        String firstLetter = "R";
        String secondLetter = "L";
        
        // Adjust these method calls if your model fields are named differently (e.g., getCity())
        if (this.getDistrict() != null && !this.getDistrict().trim().isEmpty()) {
            firstLetter = this.getDistrict().trim().substring(0, 1).toUpperCase();
        }
        
        if (this.getArea() != null && !this.getArea().trim().isEmpty()) {
            secondLetter = this.getArea().trim().substring(0, 1).toUpperCase();
        }
        
        // Pads the numeric listing ID with zeroes up to 5 spaces (e.g., 103 -> 00103)
        return String.format("%s%s-%05d", firstLetter, secondLetter, this.getListingId());
    }
    // ==========================================
    // END: CUSTOM LISTING CODE LOOK ENHANCEMENT
    // ==========================================
